...
- 핵심 개념: Path, with open, csv, 인코딩, try/except, 오류 메시지
- 실습: **CSV 성적 파일 읽기와 검증**
- 산출물: **오류에 견고한 CSV 처리 프로그램**
...
[이번 주 Colab 열기](https://colab.research.google.com/github/MNU-AI-Programming/AI-Assisted-Python-Programming/blob/main/weeks/week06/practice.ipynb)
...
![안전한 CSV 처리 흐름](../../assets/diagrams/week06-flow.svg)
...
path = find_data("scores.csv")
with path.open(encoding="utf-8") as file:
    rows = list(csv.DictReader(file))
scores = [int(row["score"]) for row in rows]
print(f"학생 수: {len(rows)}, 평균: {sum(scores)/len(scores):.1f}")
assert len(rows) >= 5
```
...
values = [safe_int(x) for x in ["10", "오류", None, "25"]]
print(values)
assert values == [10, 0, 0, 25]
```
