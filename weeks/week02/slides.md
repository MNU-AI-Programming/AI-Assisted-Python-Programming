...
- 핵심 개념: 변수, 자료형, 형 변환, 연산자, f-string, 입력값 검증의 기초
- 실습: **여행 경비 계산기**
- 산출물: **입력과 계산을 분리한 생활 문제 계산기**
...
[이번 주 Colab 열기](https://colab.research.google.com/github/MNU-AI-Programming/AI-Assisted-Python-Programming/blob/main/weeks/week02/practice.ipynb)
...
![입력값이 계산 결과가 되는 과정](../../assets/diagrams/week02-flow.svg)
...
```python
transport = 85000
lodging_per_night = 65000
nights = 2
meals_per_day = 30000
days = 3
total = transport + lodging_per_night * nights + meals_per_day * days
per_day = total / days
print(f"총예산: {total:,}원")
print(f"하루 평균: {per_day:,.0f}원")
assert total == 305000
```
...
sample = parse_positive_number("12500")
print(f"검증된 금액: {sample:,.0f}원")
```
