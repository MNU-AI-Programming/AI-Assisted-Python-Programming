...
- 핵심 개념: 최소 재현 예제, traceback, 테스트 케이스, 함수 분해, 네이밍, 회귀 방지
- 실습: **결함 있는 분석 코드 진단과 리팩토링**
- 산출물: **오류 보고서, 테스트, 개선 전후 코드**
...
[이번 주 Colab 열기](https://colab.research.google.com/github/MNU-AI-Programming/AI-Assisted-Python-Programming/blob/main/weeks/week14/practice.ipynb)
...
![AI 디버깅과 리팩토링 순환](../../assets/diagrams/week14-flow.svg)
...
```python
value = "10"
# print(value + 5)  # TypeError가 발생하는 원래 코드
fixed = int(value) + 5
print("수정 결과:", fixed)
```
...
def calculate_average(values):
    if not values:
        raise ValueError("빈 목록의 평균은 계산할 수 없습니다.")
    if not all(isinstance(v, (int, float)) for v in values):
        raise TypeError("모든 값은 숫자여야 합니다.")
...
def normalize_category(text):
    mapping = {"식 사": "식비", "meal": "식비", "bus": "교통"}
    key = str(text).strip().lower()
    return mapping.get(key, str(text).strip())

cases = {" 식 사 ": "식비", "MEAL": "식비", "bus": "교통", "도서": "도서"}
