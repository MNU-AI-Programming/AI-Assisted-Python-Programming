...
- 핵심 개념: ndarray, shape, dtype, 벡터 연산, 불리언 인덱싱, 통계
- 실습: **센서 데이터의 빠른 요약**
- 산출물: **배열 기반 기초 통계와 이상값 탐지**
...
[이번 주 Colab 열기](https://colab.research.google.com/github/MNU-AI-Programming/AI-Assisted-Python-Programming/blob/main/weeks/week09/practice.ipynb)
...
![NumPy 배열로 데이터를 계산하는 과정](../../assets/diagrams/week09-flow.svg)
...
sensor = np.array([22.1, 22.4, 23.0, 24.8, 25.2, 23.7])
print("평균:", round(sensor.mean(), 2))
print("표준편차:", round(sensor.std(), 2))
print("24도 초과:", sensor[sensor > 24])
...
```python
matrix = np.array([[80, 90, 70], [95, 85, 88], [72, 76, 81]])
student_means = matrix.mean(axis=1)
subject_means = matrix.mean(axis=0)
print("학생별 평균:", student_means)
print("과목별 평균:", subject_means)
assert len(student_means) == 3
```
