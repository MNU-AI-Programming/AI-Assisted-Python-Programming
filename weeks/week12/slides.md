...
- 핵심 개념: 질문-지표-차트 연결, EDA, 가설, 편향, 데이터 사전, 프로젝트 범위
- 실습: **지출 데이터 EDA와 연구 질문 만들기**
- 산출물: **최종 프로젝트 제안서와 EDA 노트북**
...
[이번 주 Colab 열기](https://colab.research.google.com/github/MNU-AI-Programming/AI-Assisted-Python-Programming/blob/main/weeks/week12/practice.ipynb)
...
![EDA에서 프로젝트 제안서까지](../../assets/diagrams/week12-flow.svg)
...
expenses = pd.DataFrame({
    "category": ["식비", "교통", "식비"],
    "amount": [9000, 1600, 14000],
})
print("총지출:", expenses["amount"].sum())
```
...
expenses = pd.read_csv(find_data("expenses.csv"), parse_dates=["date"])
category = expenses.groupby("category", as_index=False)["amount"].sum().sort_values("amount", ascending=False)
monthly = expenses.assign(month=expenses["date"].dt.to_period("M").astype(str)).groupby("month")["amount"].sum()
print(category)
print(monthly)
assert expenses["amount"].ge(0).all()
```
...
```python
analysis_questions = [
    "어떤 지출 카테고리의 합계가 가장 큰가?",
    "월별 총지출은 어떻게 변하는가?",
    "고액 지출 기준을 넘는 항목은 무엇인가?",
]
