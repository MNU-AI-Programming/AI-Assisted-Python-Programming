...
- 핵심 개념: 요구사항 분석, 함수·컬렉션·파일 통합, 테스트, AI 사용 내역 설명
- 실습: **성적 분석 미니 프로젝트**
- 산출물: **중간 실기 노트북과 1쪽 기술 메모**
...
[이번 주 Colab 열기](https://colab.research.google.com/github/MNU-AI-Programming/AI-Assisted-Python-Programming/blob/main/weeks/week08/practice.ipynb)
...
![중간 실기 문제 해결 순서](../../assets/diagrams/week08-flow.svg)
...
def load_scores(path):
    with path.open(encoding="utf-8") as file:
        return [{**row, "score": int(row["score"])} for row in csv.DictReader(file)]

records = load_scores(find_data("scores.csv"))
average = sum(r["score"] for r in records) / len(records)
top = max(records, key=lambda r: r["score"])
print(f"평균 {average:.1f}, 최고 {top['name']} {top['score']}점")
...
```python
def major_averages(records):
    grouped = {}
    for row in records:
        grouped.setdefault(row["major"], []).append(row["score"])
    return {key: round(sum(values) / len(values), 1) for key, values in grouped.items()}

summary = major_averages(records)
print(summary)
assert summary
```
